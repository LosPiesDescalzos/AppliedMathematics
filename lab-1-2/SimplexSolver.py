...
class SimplexSolver:
    ...
    def __init__(self, source=None, point=None, verbose=False):
        ...
        if source is not None and point is not None:
            ...
            self.n = len(source) # строки
            self.m = len(source[0]) # столбцы
            self.num_arg = self.m - 1 # количество столбцов -1 (без b)
            opt_func = source[self.n - 1] # количество строк -1 (без функции)
    ...
    def calculate(self):
        ...
        while not self.is_end(iteration):
            ...
            if main_row == -1:
                print("Не удалось выбрать разрешающий элемент. Задача не имеет решений, так как ОДР не ограничена")
                sys.exit()
            ...
            #считаем как по гауссу
            self.make_step(main_row, main_col)
            if self.verbose:
                self.print_table()

        result = [0 for _ in range(self.num_arg)]  # num_arg - количество столбцов -1 (без b)
    ...
    def make_step(self, main_row, main_col):
        ...
        # бегает по строкам пока не найдет разрешающий элемент
        for j in range(self.m):
            new_table[main_row][j] = self.table[main_row][j] / self.table[main_row][main_col]

        # бегает по столбцу разрешающего элемента и теперь зануляет весь столбец, кроме разрешающего
    ...
    def get_first_plan(self, source, basis):
        ...
        for basis_arg in basis:
            main_col = basis_arg
            main_row = -1
            min_test = -100000000000000000000
            for i in range(self.n - 1):
                if self.basis[i] == -1 and self.table[i][main_col] > min_test:
                    main_row = i
                    min_test = self.table[i][main_col]
